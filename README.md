...
| `docs/` | 链规格、rt 文件格式、锚定 API 文档、版本迭代路线 |
...
| 2026.06 | **首个完整实现落地**：链式打章 → 封章 → 离线验证 → 自包含证书全链路打通；同步完成 11 项发明专利布局，001 母案提交专利代理机构 |
| 2026.07 | **模块化重构**：打章核心 `stamp.js` 独立成模块，密码学工具库 `rt-crypto.js` 分层，写作框 / 验证器 / 锚定服务解耦；正式更名 **RealTrace**；官方创世链锚定服务上线 |
| 2026.08 | **开源 MVP v0.1.0 发布**（AGPL-3.0 + 商业双许可）；联合授权支链分发模式定案——平台获得专属支链、可离线签发、双向溯源、按链计费 |

---

## MVP 之外，还在做什么

本仓库是 **MVP 最小版**：以最少的代码实现「写作 → 打章 → 封章 → 验证」完整闭环。核心之外的迭代方向已经定好，全部保持**向后兼容**——旧版本生成的 `.rt` 链文件在后续版本必须可验证、可追溯，且每个版本号都会随链记录。

| 版本 | 技术功能 | 商业定位 |
|:-----|:---------|:---------|
| **v0.1.0（MVP，当前）** | 实时打章、防篡改链、离线验证、`.rt` 导出、创世链锚定（个人免费） | 个人免费引流；建立「人类创作认证」品牌信任 |
| **v1.0** | 轻量续写（轻量头 + 增量追加，不上传整链）、链管理（合并 / 拆分 / 分叉检测） | 提高创作连续性，增强用户粘性 |
| **v1.1** | TSA 时间戳锚定（司法级时间证明）、人脸身份锚定 | 付费分层：Basic 免费 / Plus 司法级时间证明 / Pro 双重锚定 |
| **v2.0** | 零知识验证（snarkjs / circom 电路）、异步密语原语 | 企业级信任门槛与技术壁垒 |
| **v3.0** | 联合授权平台 SDK、支链分发、按链计费、平台水印 / 人脸扩展 | B 端授权收入，平台生态 |

**版本与链上溯源**：每个 `.rt` 包内 `meta.json` 记录 `appVersion`，`chain.json` 记录链格式 `version`；封章上链时携带版本号——任何人可通过链查询到该链「何时、以哪个版本」封章。新增格式采用增量兼容：验证器先读版本号，再按对应规则解析，未知版本拒绝、已存链不受影响。

**联合授权（v3.0，已定案）**：授权平台获得由官方派生的**专属支链私钥**，可离线签发自己的子链；任何落在支链下的链都能双向回溯——向上到官方创世链、归属到平台。对平台**按链计费**（锚定即计费点），既随规模增长，也天然抑制恶意刷链。
