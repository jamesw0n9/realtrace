...
| 2026.08 | **开源 MVP v0.1.0 发布**（AGPL-3.0 + 商业双许可）；联合授权支链模式进入规划 |

---

## MVP 之外，还在做什么

本仓库是 **MVP 最小版**：以最少的代码实现「写作 → 打章 → 封章 → 验证」完整闭环。更远的方向已经画出轮廓，但真正的产品是迭代出来的——**每个版本只有正式发布后，才会在这里写「做到了什么」**。所有迭代保持向后兼容：旧版本生成的 `.rt` 链文件在后续版本必须可验证、可追溯，且版本号随链记录。

| 版本 | 探索方向 | 价值想象 |
|:-----|:---------|:---------|
| **v0.1.0（MVP，已发布）** | 实时打章、防篡改链、离线验证、`.rt` 导出、创世链锚定（个人免费） | 个人免费，先让「人类创作在场证明」被看见、被用起来 |
| **v1.0** | 轻量续写（轻量头 + 增量追加，不上传整链）、链管理（合并 / 拆分 / 分叉检测） | 长文连续创作，链越长、证据越厚 |
| **v1.1** | TSA 时间戳锚定、人脸身份锚定 | 让一份创作证明从「可信」走向「可司法采信」 |
| **v2.0** | 零知识验证、异步密语原语 | 在不暴露任何内容的前提下，证明创作的真实性 |
| **v3.0** | 联合授权平台 SDK、支链分发、平台水印 / 人脸扩展 | 平台生态：让第三方平台也能签发可溯源的真迹链 |
...
**联合授权（方向）**：我们为平台合作预留了**专属支链**的想象空间——平台签发的每一份创作证明，都能独立向上溯源到官方创世链、向下归属到平台。具体能力与开放节奏，将在对应版本发布时正式揭晓。
